...
PROTOCOL_STATUS = "frozen_before_toolsandbox_v42_training"


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--method", choices=("mask", "v42"), required=True)
    ...
    parser.add_argument("--presentations-per-epoch", type=int, default=36)
    ...
    if protocol.get("status") != PROTOCOL_STATUS:
        raise ValueError("invalid ToolSandbox V4.2 protocol status")
    if args.method not in protocol.get("methods", []):
        raise ValueError("method is absent from the frozen V4.2 comparison")
    config = {
        "seed": args.seed,
        "epochs": args.epochs,
    ...
    if config != protocol.get("config"):
        raise ValueError("training CLI does not match the frozen V4.2 config")
    ...
    if not source_hashes or not all(
        ...
        raise ValueError("V4.2 source identity changed after protocol freeze")
    ...
    expected = protocol.get("expected_presented_event_sequence_sha256")
    if sequence_sha256 != expected:
        raise RuntimeError("actual event presentation sequence differs from protocol")
    ...
    if dict(sorted(presented_decisions.items())) != expected_labels:
        raise RuntimeError("actual training labels differ from protocol")
    ...
    summary = {
        "status": "completed",
        "stage": "toolsandbox_v42_balanced_margin_training",
        "method": args.method,
        "seed": args.seed,
