...
def main() -> None:
    ...
    if protocol.get("status") != "frozen_before_toolsandbox_v41_preference_outcomes":
        raise ValueError("invalid frozen comparison protocol")
    ...
    with torch.no_grad():
        for public in public_rows:
            ...
            target = (
                "b" if private["decision"] == "rescue_preference" else "a"
            )
    ...
    metrics = summarize_evaluation_rows(results)
    summary = {
        "status": "completed",
        "stage": "toolsandbox_v41_preference_evaluation",
        "method": args.method,
        ...
        **metrics,
        ...
        "scope": "fresh controlled-state ToolSandbox preference diagnostic; not autonomous task success",
