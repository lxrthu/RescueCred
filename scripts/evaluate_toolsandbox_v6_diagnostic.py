...
from rescuecredit.toolsandbox_router import desired_candidate, summarize_router_predictions
...
def main() -> None:
    ...
    parser.add_argument(
        "--method",
        choices=("default_b", "margin_probe", "semantic_probe"),
        required=True,
    )
    ...
    if protocol.get("status") != PROTOCOL_STATUS:
        raise ValueError("invalid V6 evaluation protocol")
    expected_lock = file_sha256(args.protocol_lock)
    if scoring.get("protocol_lock_sha256") != expected_lock:
        raise ValueError("V6 scoring was not bound to this protocol")
    if scoring.get("private_outcomes_read") is not False:
        raise ValueError("V6 public-only scoring boundary was violated")
    if scoring.get("evaluation_role") != args.evaluation_role:
        raise ValueError("V6 scoring/evaluation role mismatch")
    if scoring.get("prediction_sha256", {}).get(args.method) != file_sha256(
        args.predictions
    ):
        raise ValueError("V6 prediction identity mismatch")
    if scoring.get("public_events_sha256") != file_sha256(args.public_events):
        raise ValueError("V6 public event identity mismatch")
    ...
    if not (
        set(public_by_id) == set(private_by_id) == set(prediction_by_id)
        and len(prediction_by_id) == len(predictions)
    ):
        raise ValueError("V6 public/private/prediction event ids differ")
        ...
        results.append(
            {
                ...
                "selected": selected,
                "target": desired_candidate(decision),
                "causal_correct": selected == desired_candidate(decision),
                "flipped": bool(prediction.get("routed_to_a", False)),
                "abstained_to_b": bool(prediction.get("abstained_to_b", False)),
                "reverse_probability": prediction.get("reverse_probability"),
    ...
    summary = {
        "status": "completed",
        "stage": "toolsandbox_v6_offline_reverse_evaluation",
        "method": args.method,
