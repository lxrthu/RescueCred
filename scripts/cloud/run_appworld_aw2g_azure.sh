...
ROOT_OUT=outputs/appworld_harness_audit_30_v7_azure
mkdir -p "$ROOT_OUT"
...
run_partition fresh_holdout 60

"$PY_APPWORLD" - <<'PY'
import json
from pathlib import Path

root = Path("outputs/appworld_harness_audit_30_v7_azure")
gates = {}
for partition in ("development", "fresh_holdout"):
...
passed = bool(gates["development"].get("passed") and gates["fresh_holdout"].get("passed"))
combined = {
    "passed": passed,
    "selector": "Azure GPT-4o, reference-free visible-candidate selection",
    "confidence_threshold_frozen_before_fresh_holdout": 0.90,
    "reference_boundary": "no reference action or protected value enters Azure prompts",
    "partitions": {
        "development": "train tasks 0:30",
        "fresh_holdout": "train tasks 60:90, unused by AW2b through AW2f",
    },
    "gates": gates,
