...
def main() -> None:
    ...
    rejected = {
        "parse_failure": 0,
        "reference_year_mismatch": 0,
        "public_tool_coverage": 0,
        "duplicate": 0,
    }
    ...
    for path in source_files:
        ...
        if task is None:
            ...
            continue
        if not reference_years_consistent(task):
            rejected["reference_year_mismatch"] += 1
            continue
        public_tools = {tool["name"] for tool in task.get("available_tools", [])}
        reference_tools = {action["tool"] for action in task.get("reference_actions", [])}
        if not reference_tools <= public_tools:
            rejected["public_tool_coverage"] += 1
            continue
    ...
    manifest = {
        ...
        "rejected": rejected,
        "available_tools_contract": {
            "construction": "source filename plus schema prerequisite closure",
            "reference_actions_used_for_construction": False,
            "reference_actions_used_for_offline_coverage_check": True,
            "all_runtime_tool_sets_reference_independent": all(
                task.get("available_tools_reference_independent") is True for task in all_tasks
            ),
        },
