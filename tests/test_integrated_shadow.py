...
def test_schema_valid_off_path_call_can_recover_later():
    ...
    _, reward, done, _ = env.step(task["reference_actions"][0])
    assert done and reward == 1.0


def test_h3_repairs_semantic_argument_mismatch():
    env = APIBankControlledEnv()
    env.reset(TASK, 9)
    proposal = {"tool": "SendEmail", "arguments": {"to": "wrong@example.com", "body": "hello"}}
    executed, decision = APIBankHarness("H3").execute(
        env.observation(), proposal, env.expected_action()
    )
    ...
    _, reward, done, info = env.step(executed)
    assert done and reward == 1.0
    assert info["ground_truth_match"] is True


def test_h3_missing_argument_repair_does_not_preserve_other_wrong_values():
    env = APIBankControlledEnv()
    env.reset(TASK, 10)
    proposal = {"tool": "SendEmail", "arguments": {"to": "wrong@example.com"}}
    executed, decision = APIBankHarness("H3").execute(
        env.observation(), proposal, env.expected_action()
    )
