...
def history_patch_id(decision: Any) -> str | None:
    ...
    Feedback-only/unknown decisions are diagnostics, not environment
    observations. Injecting internal patch ids into ``history`` changes later
    policy actions while the run still reports zero execution interventions.
    """
